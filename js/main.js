...
;(function(window) {
	...
	PieceMaker.prototype.options = {
		...
		tilt: {maxRotationX: 5, maxRotationY: 7, maxTranslationX: 4, maxTranslationY: -2}
	};
	...
	DOM.gallery= document.querySelector('.kuang .innerclicker');
	DOM.loading = document.querySelector('.loading');
	DOM.switchCtrls = document.querySelector('.switch');
	DOM.kuangkuang = document.querySelector('.kuang .outer');/*'{
		'design' : document.querySelector('.kuang.outer.mode--design'),
		'code' : document.querySelector('.kuang.outer.mode--code')
	};*/
	DOM.switchModeCtrls = {
		'design' : DOM.switchCtrls.firstElementChild,
		'code' : DOM.switchCtrls.lastElementChild
	};
	DOM.pieces = document.querySelector('.kuang > .pieces');
	...
	DOM.logo = document.querySelector('.watermark');
	// The current mode.
	let mode = 'design', disablePageFx, isAnimating;

	function hoverWatermark(){
	...
	function initEvents() {
		DOM.gallery.addEventListener('click', galleryEntering);
	...
	function switchContent() {
		...
		const dl = 'mode--' + (mode === 'code' ? 'design' : 'code');
		const ad = 'mode--' + mode;
		// DOM.switchCtrls.classList.remove('mode--' + (mode === 'code' ? 'design' : 'code'));
		// DOM.switchCtrls.classList.add('mode--' + mode);
		DOM.switchCtrls.classList.remove(dl);
		DOM.switchCtrls.classList.add(ad);

		DOM.kuangkuang.classList.remove(dl);
		DOM.kuangkuang.classList.add(ad);

		DOM.body.classList.remove(dl);
		DOM.body.classList.add(ad);
